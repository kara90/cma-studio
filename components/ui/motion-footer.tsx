...
/**
 * CinematicFooter — a GSAP "curtain reveal" footer with magnetic pills, a
 * diagonal marquee, giant background wordmark, and glass buttons. Adapted for
 * CMA Studio Pro: brand fonts (no external @import — our CSP blocks it), CMA
 * CTAs, a "Powered by CineMaster Academy" credit, and a Contact link.
 */
...
const CONTACT = 'mailto:hello@cinemasteracademy.com?subject=CMA%20Studio%20Pro';
...
.footer-bg-grid {
  ...
  background-image:
    linear-gradient(to right, color-mix(in oklch, var(--primary) 16%, transparent) 1px, transparent 1px),
    linear-gradient(to bottom, color-mix(in oklch, var(--primary) 16%, transparent) 1px, transparent 1px);
  mask-image: linear-gradient(to bottom, transparent, black 12%, black 88%, transparent);
  -webkit-mask-image: linear-gradient(to bottom, transparent, black 12%, black 88%, transparent);
}
.footer-aurora { background: radial-gradient(circle at 50% 50%, color-mix(in oklch, var(--primary) 18%, transparent) 0%, color-mix(in oklch, var(--secondary) 15%, transparent) 40%, transparent 70%); }
.footer-glass-pill {
...
.footer-giant-bg-text {
  ...
  font-size: 26vw; line-height: 0.75; font-weight: 800; letter-spacing: -0.05em; color: transparent;
  -webkit-text-stroke: 1px color-mix(in oklch, var(--foreground) 6%, transparent);
  background: linear-gradient(180deg, color-mix(in oklch, var(--primary) 12%, transparent) 0%, transparent 60%);
...
const MARQUEE = ['Real film hardware', 'Auto-Direct', 'Anamorphic · 70mm · Vintage glass', 'Your key · your control', 'Fewer wasted renders', 'Cinematic by default'];
  ...
  <div className="flex items-center space-x-12 px-6">
    {MARQUEE.map((m, i) => (
      <React.Fragment key={m}>
        <span>{m}</span>
        <span className={i % 2 ? 'text-secondary/60' : 'text-primary/60'}>✦</span>
      </React.Fragment>
    ))}
  </div>
);
    ...
    <>
      ...
      <div ref={wrapperRef} className="relative h-screen w-full" style={{ clipPath: 'polygon(0% 0, 100% 0%, 100% 100%, 0 100%)' }}>
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-background text-foreground cinematic-footer-wrapper">
          ...
          <div ref={giantTextRef} className="footer-giant-bg-text absolute -bottom-[4vh] left-1/2 -translate-x-1/2 whitespace-nowrap z-0 pointer-events-none select-none">
            CINEMASTER
          </div>
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-20 w-full max-w-5xl mx-auto">
            <h2 ref={headingRef} className="text-5xl md:text-8xl font-extrabold footer-text-glow tracking-tighter mb-12 text-center">
              Ready to direct?
            </h2>
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-muted-foreground text-[10px] md:text-xs font-semibold tracking-widest uppercase order-2 md:order-1">
              © 2026 CineMaster Academy · CMA Studio Pro
            </div>

            <a
              href="https://cinemasteracademy.com"
              target="_blank"
              rel="noopener"
              className="footer-glass-pill px-6 py-3 rounded-full flex items-center gap-2 order-1 md:order-2 border-border/50"
            >
              <span className="text-muted-foreground text-[10px] md:text-xs font-bold uppercase tracking-widest">Powered by</span>
              <span className="text-foreground font-black text-xs md:text-sm tracking-normal">CineMaster Academy</span>
            </a>
